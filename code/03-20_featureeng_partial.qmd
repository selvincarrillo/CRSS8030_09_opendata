...
```
The mean and median should be close if the data is normal. 
...
In the case of precipitation, we can use the sum of total precipitation per month, year, etc. For temperature it makes more sense to summarize by mean every week, month, etc.  

Feature engineering means creating new variables from the ones that we have, like mean or median of temperature by week or month instead of daily. Or create a Growing Degree Days. 
...
fe_month <- fieldweather %>%
  ...
  mutate(date = as.Date(date_chr, "%Y/%j")) %>%  # for as.Date give the date and then the format(look at the link above)
...
fe_month_sum <- 
  fe_month %>% 
  group_by(year, site, month_abb, strength_gtex) %>% # keep strength_gtx even though is a response variable
  summarise(across(.cols = c(dayl.s,
                             srad.wm2, 
                             tmax.c, 
                             tmin.c, 
                             vp.pa), 
                   .fns = mean, # \(x) mean (x, na.rm = TRUE) to remove NA's 
                   .names = "mean_{.col}"), # specify the column formate
            across(.cols = prcp.mm, 
                   .fns = sum, 
                   .names = "sum_{.col}")
            ) %>% 
...
Let's check tmax.c and prcp.mm for the first site-year and month.  This is to double check the values we got above for just one year, site, and month. 
