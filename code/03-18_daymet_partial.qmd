...
library(sf) # for US map and geospatial data 
library(daymetr) # API to access database
```
...
The data contains the following columns:  
  ...
  - **strength_gtex**: cotton fiber strength (in g/tex) as quality measure; higher gtx means stronger fiber. Data is strength for each location but it is not accounting for any specific variety. We want to predict this variable.  
...
field %>% 
  ...
  geom_density() + # or use geom_histogram() or geom_boxplot 
  geom_rug()
```

Create a map of the US and add the points (Spatial distribution)   
```{r}
# boundaries of the US 
states <- us_states() %>% 
  filter(!(state_abbr %in%  c("PR", "AK", "HI"))) #we don not want these states for now
...
Ideally, the best weather station data is an in-house, then a State network station, and last an open-source database like Daymet. Daymet allows us to access data from many years back.  
...
daymet_one <- download_daymet(site = field$site[[1]], 
                              ...
                              simplify = T) # more friendly looking table 
...
```{r}
daymet_one %>%  # looks like the table is in pivot_longer() format 
...
```{r}
daymet_all <- 
  field %>%  # we do not need to group_by and nest() because it will gives the same row number
  mutate(weather = pmap(list(.y = year, 
                        ...
                        function(.y, .site, .lat, .lon)
                          download_daymet(
                            ...
                            simplify = T, 
                            silent = T  ) %>% # inhibits progress messages
                          rename(.year = year,  # rename so we do not get duplicate columns
                                 .site = site)
                        ))
...
daymet_all_unnest <- 
  daymet_all %>% 
  unnest(weather) %>% #unnest the tibbles hidden 
  pivot_wider(names_from = measurement, # days * sites
              values_from = value) %>% 
  janitor::clean_names()
...
Let's do that below:  
```{r}
# export the data so we can work on it 
...
Some of the ones I used in the past:  
  ...
  - Soil properties:  
    - **POLARIS**: https://github.com/lhmrosso/XPolaris  For the US
    - **Soilsgrid**: https://rpubs.com/ials2un/soilgrids_webdav  For the wolds
  ...
  - **Elevation**: https://github.com/jhollist/elevatr  for on farm studies or determine how to place blocks 
